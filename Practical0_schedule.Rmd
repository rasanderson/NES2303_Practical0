...
You are going to use the `mosaic` package for the first few practicals. You only have to install it once, and it will auto-install some additional packages in the process. In the Console window type the following text below, followed by the Enter key:
...
You will probably see some text displayed warning you that various 'objects are masked'. This is simply to tell you that the `mosaic` package loads additional commands, that have the same names as some basic ones, and is nothing to worry aboue.
...
Now save your R script. Click on **File -> Save** from the RStudio main menu, or if you prefer the save icon, and save the script as `practical0.R`. Next run each line of your script. Click on the `library(mosaic)` line and hit the <kbd>Ctl</kbd> + <kbd>Enter</kbd> keys simultaneously. The Enter key is the large one at the right of the alphabetic keyboard. This key combination sends a line from your script to the Console window where it is executed. The command should run without displaying anything as `mosaic` is probably already activated. Your cursor should jump to the `read.csv` line.
...
So, finally we need to convert our Microsoft Excel file. Go back to your File Manager, navigate to the NES2303 folder (it might already be open) and double-click on the `yield` file to open it in Microsoft Excel. Then click on the **File -> Save As** option (or **File -> Save a Copy** depending on Excel version), and in the drop-box below the filename, select the CSV option:
...
You probably won't use this very often, but you can easily enter values into the **Console** window (bottom-left) if you need to do some quick arithmetic:
...
my_data_object <- read.csv(file = "Path_to_the_file.csv")
...
write.csv(my_data_object, file = "Data/my_output_data.csv")
```
